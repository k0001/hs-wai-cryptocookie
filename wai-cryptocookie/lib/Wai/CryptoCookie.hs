...
--
-- * Encoding and decoding of @aad@ and @msg@ are done through 'Ae.ToJSON'
--   and 'Ae.FromJSON'.
--
...
--     'Key' and associated with a specific @aad@. In principle, this makes this
--     encryption scheme suitable for storing user session authentication
--     identifiers generated by the server. However, in order to prevent
--     cross-site request forgeries, please consider using something like
--     "Wai.CSRF".'Wai.CSRF.Token' as your @aad@, ensuring that you rotate said
--     'Wai.CSRF.Token' ocassionally, at least each time a new user session is
--     established, so as to avoid CSRF issues.
defaultConfig
   :: (Ae.ToJSON aad, Ae.ToJSON msg, Ae.FromJSON msg)
